 ...
 */
export function formatAsDate(dateString: string) {
 ...
 */
export function formatAsTime(timeString: string) {
 ...
 */
export function previous(currentDate: string) {
 ...
 */
export function next(currentDate: string) {
  // eslint-disable-next-line prefer-const
  let [ year, month, day ] = currentDate.split("-").map(Number);
  month -= 1;
  const date = new Date(year, month, day);
