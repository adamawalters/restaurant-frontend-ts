...
function Menu(): JSX.Element {
  ...
  const search = useRef<HTMLAnchorElement | null>(null)
  const newReservation = useRef<HTMLAnchorElement | null>(null)
  const newTable = useRef<HTMLAnchorElement | null>(null)
  ...
  useEffect(() => {
    ...
    refs.forEach((ref) => {
      if (ref.current) {
        ref.current.className = `nav-link text-white`;
      }
    });

    if (pathname.startsWith("/dashboard")) {
      dashboard.current? dashboard.current.className = `nav-link active` : null
    } else if (pathname.startsWith("/search")) {
      search.current? search.current.className = `nav-link active` : null
    } else if (pathname.startsWith("/reservations/new")) {
      newReservation.current ? newReservation.current.className = `nav-link active`: null
    } else if (pathname.startsWith("/tables/new")) {
      newTable.current ? newTable.current.className = `nav-link active`: null
    }
  }, [pathname]);
      ...
      <div className="container-fluid flex-column p-0">
        ...
        >
          ...
          <li className="nav-item mx-1 mx-md-0 my-1 my-md-0">
            <Link className="nav-link text-white" to="/search" ref={search}>
